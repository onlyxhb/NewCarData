...
import com.xhb.onlystar.bean.Date;
...
public class MonthDateView extends View {
    ...
    private void drawCircle(int row, int column, int day, Canvas canvas) {
        if (daysHasThingList != null && daysHasThingList.size() > 0) {
            for (Date MDate : daysHasThingList) {
                if (MDate.getYear() != mSelYear) return;
                if (MDate.getMonth() != mSelMonth+1) return;
                if (MDate.getDay() != day) return;
                else {
     ...
     */
    public void setDaysHasThingList(List<Date> daysHasThingList) {
